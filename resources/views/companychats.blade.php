@foreach($ch->where('student_id', $student->id)->where('company_id', session('id'))->where('status', '!=', 'sentbystudent')->get() as $chat)                
  @if($chat->company_id == session('id') && $chat->sendertype == session('type'))
    <div class="row chats">
